...
class Kategori extends CI_Controller
{
	...
	function __construct()
	{
		...
		auth_admin();
		$this->load->model(admin_folder() . '/kategori_model');

        $this->folder        = admin_folder();
        $this->ctrl          = 'master/kategori';
        $this->template      = admin_folder() . '/master/kategori/';
        $this->path_uri      = admin_folder() . '/master/kategori';
        $this->path          = base_url() . admin_folder().'/master/kategori';
        $this->title         = get_admin_menu_title('kategori');
        $this->id_admin_menu = get_admin_menu_id('kategori');
	...
	function main()
	{
        ...
        $no = $lmt;
        $total_records = $this->kategori_model->getTotal($s_name);
        $query = $this->kategori_model->getAll($s_name, $per_page, $lmt);
        ...
        $this->parser->parse($template.'kategori', $data);
    ...
    public function add()
    {
        if ($_SERVER['REQUEST_METHOD'] == 'POST' && $this->validateForm()) {
            ...
            $result = $this->kategori_model->insert($post);            
    ...
    public function edit($id = "")
    {
        ...
        if ( ($_SERVER['REQUEST_METHOD'] == 'POST') && $this->validateForm($id) ) {
            ...
            $result = $this->kategori_model->update($id, $post);
    ...
    {
        $result = $this->kategori_model->status($id, $status);
    ...
    private function getForm($id = "")
    {
        ...
        if ($id) {
            $query = $this->kategori_model->getById($id);
            if ($query->num_rows()>0) {
                $row = $query->row_array();
            } else {
        ...
        $this->parser->parse($template . 'kategori_form', $data);
        $this->global_libs->print_footer();
    }
        ...
        if ($this->input->post('id')!= '') {
            if (strlen($this->input->post('id')) > 0) {
                    ...
                    if ($this->kategori_model->delete($id)) {
                        $this->session->set_flashdata('success_msg', 'kategori has been deleted.');
                    } else {
