...
class Barang extends CI_Controller
{
	...
	function __construct()
	{
        ...
        $this->ctrl          = 'master/barang';
        $this->template      = admin_folder() . '/master/barang/';
        $this->path_uri      = admin_folder() . '/master/barang';
        $this->path          = base_url() . admin_folder().'/master/barang';
        $this->title         = get_admin_menu_title('master/barang');
	...
	function main()
	{
        ...
        // $query = $this->barang_model->getAll($s_name, $per_page, $lmt);
        $query = $this->barang_model->getAll();
	...
	private function getForm($id = "")
    {
        ...
        if ($id) {
            $query = $this->barang_model->getById($id);
            if ($query->num_rows()>0) {
                $row = $query->row_array();
            } else {
        ...
        $this->parser->parse($template . 'barang_form', $data);
        $this->global_libs->print_footer();
    }
	...
	public function add()
    {
        if ($_SERVER['REQUEST_METHOD'] == 'POST' && $this->validateForm()) {
            $idbarang = "";
            $total = auto_inc('barang');
            $idbarang = "BG".sprintf("%04d", $total);
            
            $post = purify($this->input->post());
            $post['idbarang'] = $idbarang;
            $this->replacer($post);

            $result = $this->barang_model->insert($post);
    ...
    public function edit($id = "")
    {
        ...
        if ( ($_SERVER['REQUEST_METHOD'] == 'POST') && $this->validateForm($id) ) {
            ...
            $result = $this->barang_model->update($id, $post);
    ...
    {
        $result = $this->barang_model->status($id, $status);
