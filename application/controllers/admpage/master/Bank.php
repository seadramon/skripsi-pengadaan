...
class Bank extends CI_Controller
{
	...
	function __construct()
	{
		...
		auth_admin();
		$this->load->model(admin_folder() . '/bank_model');

        $this->folder        = admin_folder();
        $this->ctrl          = 'master/bank';
        $this->template      = admin_folder() . '/master/bank/';
        $this->path_uri      = admin_folder() . '/master/bank';
        $this->path          = base_url() . admin_folder().'/master/bank';
        $this->title         = get_admin_menu_title('bank');
        $this->id_admin_menu = get_admin_menu_id('bank');

        $this->max_width    = 281;
        $this->max_height   = 392;
	}

    private $arrForm = array('swift_code',
                         'nama',
                         'status');
    private $error;
	...
	function main()
	{
        ...
        $no = $lmt;
        $total_records = $this->bank_model->getTotal($s_name);
        $query = $this->bank_model->getAll($s_name, $per_page, $lmt);
        ...
        $this->parser->parse($template.'bank', $data);
    ...
    public function add()
    {
        if ($_SERVER['REQUEST_METHOD'] == 'POST' && $this->validateForm()) {
            ...
            $post['swift_code'] = strtoupper($post['swift_code']);
            $post['nama'] = ucwords($post['nama']);

            $result = $this->bank_model->insert($post);
    ...
    public function edit($id = "")
    {
        ...
        if ( ($_SERVER['REQUEST_METHOD'] == 'POST') && $this->validateForm($id) ) {
            ...
            $result = $this->bank_model->update($id, $post);
    ...
    {
        $result = $this->bank_model->status($id, $status);
    ...
    private function getForm($id = "")
    {
        ...
        if ($id) {
            $query = $this->bank_model->getById($id);
            if ($query->num_rows()>0) {
                $row = $query->row_array();
            } else {
        ...
        $this->parser->parse($template . 'bank_form', $data);
        $this->global_libs->print_footer();
    }
