...
class Penerimaan extends CI_Controller
{
	...
	public function index()
	{
		...
		if ($_SERVER['REQUEST_METHOD'] == 'POST') {
			...
			if ($post['start']!="" && $post['end']!="") {
				$data = $this->laporan_model->penerimaan($post['start'], $post['end']);
				$start = $post['start'];
				$end = $post['end'];
			} else {
		...
		$this->load->view('admpage/laporan/penerimaan/penerimaan', $data);
	...
	public function cetak($start, $end)
	{
		...
		$data = $this->laporan_model->penerimaanPdf($start, $end);
		$arrData = ['data' => $data,
			'start' => $start,
			'end'	=> $end,
		];
		$html = $this->load->view('admpage/laporan/penerimaan/penerimaanPdf', $arrData, true);
		...
		$this->mpdf->Output('penerimaan.pdf', 'I');
	}
}