...
class Barangkeluar_model extends CI_Model
{
    ...
    function insertDetail($data)
    {
        ...
        try {
            if (is_array($data) && count($data) > 0) {
                if ($this->db->insert_batch('detail_barangkeluar', $data)) {
                    $result = 1;
                }
            }
        } catch (Exception $e) {
    ...
    function clearDetail($id)
    {
        ...
        if ($this->db->delete('detail_barangkeluar')) {
            $result = true;
        }
    ...
    function getTotal($name = null)
    {
        ...
        $this->db->select('COUNT(idbarangkeluar) as total');
    ...
    {
        $this->db->select('barangkeluar.*, detail_barangkeluar.*');
        $this->db->from($this->table);
        $this->db->join('detail_barangkeluar', $this->table.'.idbarangkeluar = detail_barangkeluar.idbarangkeluar', 'left');

        $this->db->where($this->table.'.idbarangkeluar', $id);
        $query = $this->db->get();
