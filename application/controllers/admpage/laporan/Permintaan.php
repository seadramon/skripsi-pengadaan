...
class Permintaan extends CI_Controller
{
	...
	public function index()
	{
		...
		if ($_SERVER['REQUEST_METHOD'] == 'POST') {
			...
			if ($post['start']!="" && $post['end']!="") {
				$data = $this->laporan_model->permintaan($post['start'], $post['end'], $post['status']);
				$start = $post['start'];
				$end = $post['end'];
		...
		$this->load->view('admpage/laporan/permintaan/permintaan', $data);
	...
	public function cetak($start, $end, $status = "")
	{
		...
		$data = $this->laporan_model->permintaanPdf($start, $end, $status);
		$arrData = ['data' => $data,
			'start' => $start,
			'end'	=> $end,
			'status' => $status
		];
		$html = $this->load->view('admpage/laporan/permintaan/permintaanPdf', $arrData, true);
