...
class Fase extends CI_Controller
{
	...
	function __construct()
	{
		...
		auth_admin();
		$this->load->model(admin_folder() . '/fase_model');

        $this->folder        = admin_folder();
        $this->ctrl          = 'master/fase';
        $this->template      = admin_folder() . '/master/fase/';
        $this->path_uri      = admin_folder() . '/master/fase';
        $this->path          = base_url() . admin_folder().'/master/fase';
        $this->title         = get_admin_menu_title('fase');
        $this->id_admin_menu = get_admin_menu_id('fase');
	...
	function main()
	{
        ...
        $no = $lmt;
        $total_records = $this->fase_model->getTotal($s_name);
        $query = $this->fase_model->getAll($s_name, $per_page, $lmt);
        ...
        $this->parser->parse($template.'fase', $data);
    ...
    public function add()
    {
        if ($_SERVER['REQUEST_METHOD'] == 'POST' && $this->validateForm()) {
            ...
            $result = $this->fase_model->insert($post);            
    ...
    public function edit($id = "")
    {
        ...
        if ( ($_SERVER['REQUEST_METHOD'] == 'POST') && $this->validateForm($id) ) {
            ...
            $result = $this->fase_model->update($id, $post);
    ...
    {
        $result = $this->fase_model->status($id, $status);
    ...
    private function getForm($id = "")
    {
        ...
        if ($id) {
            $query = $this->fase_model->getById($id);
            if ($query->num_rows()>0) {
                $row = $query->row_array();
            } else {
        ...
        $this->parser->parse($template . 'fase_form', $data);
        $this->global_libs->print_footer();
    }
    ...
    public function delete()
    {
        if ($this->input->post('id')!= '') {
            if (strlen($this->input->post('id')) > 0) {
                    ...
                    if ($this->fase_model->delete($id)) {
                        $this->session->set_flashdata('success_msg', 'fase has been deleted.');
                    } else {
