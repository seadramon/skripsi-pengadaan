...
class Satuan extends CI_Controller
{
	...
	function __construct()
	{
		...
		auth_admin();
		$this->load->model(admin_folder() . '/satuan_model');

        $this->folder        = admin_folder();
        $this->ctrl          = 'master/satuan';
        $this->template      = admin_folder() . '/master/satuan/';
        $this->path_uri      = admin_folder() . '/master/satuan';
        $this->path          = base_url() . admin_folder().'/master/satuan';
        $this->title         = get_admin_menu_title('satuan');
        $this->id_admin_menu = get_admin_menu_id('satuan');
    ...
    private $arrForm = array('nama', 'id_tipe');
    private $error;
	...
	function main()
	{
        ...
        $no = $lmt;
        $total_records = $this->satuan_model->getTotal($s_name);
        $query = $this->satuan_model->getAll($s_name, $per_page, $lmt);
        ...
        $this->parser->parse($template.'satuan', $data);
    ...
    public function add()
    {
        if ($_SERVER['REQUEST_METHOD'] == 'POST' && $this->validateForm()) {
            ...
            $result = $this->satuan_model->insert($post);            
    ...
    public function edit($id = "")
    {
        ...
        if ( ($_SERVER['REQUEST_METHOD'] == 'POST') && $this->validateForm($id) ) {
            ...
            $result = $this->satuan_model->update($id, $post);
    ...
    {
        $result = $this->satuan_model->status($id, $status);
    ...
    private function getForm($id = "")
    {
        ...
        // Form attribute
        $tipe = get_combobox("SELECT id_tipe, nama FROM tipe", "id_tipe", "nama", "-Pilih Tipe-");
        ...
        if ($id) {
            $query = $this->satuan_model->getById($id);
            if ($query->num_rows()>0) {
                $row = $query->row_array();
            } else {
        ...
        $this->parser->parse($template . 'satuan_form', $data);
        $this->global_libs->print_footer();
    }
        ...
        if ($this->input->post('id')!= '') {
            if (strlen($this->input->post('id')) > 0) {
                    ...
                    if ($this->satuan_model->delete($id)) {
                        $this->session->set_flashdata('success_msg', 'satuan has been deleted.');
                    } else {
