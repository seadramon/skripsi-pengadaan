...
class Supplier extends CI_Controller
{
	...
	function __construct()
	{
		parent::__construct();
		auth_admin();
		$this->load->model(admin_folder() . '/supplier_model');

        $this->folder        = admin_folder();
        $this->ctrl          = 'master/supplier';
        $this->template      = admin_folder() . '/master/supplier/';
        $this->path_uri      = admin_folder() . '/master/supplier';
        $this->path          = base_url() . admin_folder().'/master/supplier';
        $this->title         = get_admin_menu_title('master/supplier');
	...
	function main()
	{
        ...
        $no = $lmt;
        $total_records = $this->supplier_model->getTotal($s_name);
        // $query = $this->supplier_model->getAll($s_name, $per_page, $lmt);
        ...
        $this->parser->parse($template.'supplier', $data);
	...
	private function getForm($id = "")
    {
        ...
        $this->parser->parse($template . 'supplier_form', $data);
        $this->global_libs->print_footer();
    }
	...
	public function add()
    {
        if ($_SERVER['REQUEST_METHOD'] == 'POST' && $this->validateForm()) {
            $idsupplier = "";
            $total = auto_inc('supplier');
            $idsupplier = "SP".sprintf("%03d", $total);
            
            $post = purify($this->input->post());
            $post['idsupplier'] = $idsupplier;
            $this->replacer($post);

            $result = $this->supplier_model->insert($post);
    ...
    public function edit($id = "")
    {
        ...
        if ( ($_SERVER['REQUEST_METHOD'] == 'POST') && $this->validateForm($id) ) {
            ...
            $result = $this->supplier_model->update($id, $post);
    ...
    {
        $result = $this->supplier_model->status($id, $status);
        ...
        if ($this->input->post('id')!= '') {
            if ( (ctype_digit($this->input->post('id'))) && ($this->input->post('id')>0)) {
                    ...
                    if ($this->bank_model->delete($id)) {
                        $this->session->set_flashdata('success_msg', 'bank has been deleted.');
                    } else {
    ...
    private function validateForm($id = 0)
    {
        ...
        foreach ($this->arrForm as $fieldName) {
            if ($post[$fieldName]=='') {
                $this->error[$fieldName] = $fieldName.' tidak boleh kosong';    
            } else {
                if ($fieldName=='telp' || $fieldName=='fax') {
                ...
                if ($fieldName=='email') {
                    if (!filter_var($post[$fieldName], FILTER_VALIDATE_EMAIL)) {
                        $this->error[$fieldName] = $fieldName.' tidak valid';
                    }
                }
            }
        }
