...
class Barangkeluar extends CI_Controller
{
	...
	public function index()
	{
		...
		if ($_SERVER['REQUEST_METHOD'] == 'POST') {
			...
			if ($post['start']!="" && $post['end']!="") {
				$data = $this->laporan_model->barangkeluar($post['start'], $post['end']);
				$start = $post['start'];
				$end = $post['end'];
			} else {
		...
		$this->load->view('admpage/laporan/barangkeluar/barangkeluar', $data);
	...
	public function cetak($start, $end)
	{
		...
		$data = $this->laporan_model->barangkeluarPdf($start, $end);
		$arrData = ['data' => $data,
			'start' => $start,
			'end'	=> $end
		];
		$html = $this->load->view('admpage/laporan/barangkeluar/barangkeluarPdf', $arrData, true);
