...
class Divisi extends CI_Controller
{
	...
	function __construct()
	{
        ...
        $this->ctrl          = 'master/divisi';
        $this->template      = admin_folder() . '/master/divisi/';
        $this->path_uri      = admin_folder() . '/master/divisi';
        $this->path          = base_url() . admin_folder().'/master/divisi';
        $this->title         = get_admin_menu_title('master/divisi');
        $this->id_admin_menu = get_admin_menu_id('divisi');
	...
	function main()
	{
        ...
        $no = $lmt;
        $total_records = $this->divisi_model->getTotal($s_name);
        // $query = $this->divisi_model->getAll($s_name, $per_page, $lmt);
        $query = $this->divisi_model->getAll();
        ...
        $this->parser->parse($template.'divisi', $data);
	...
	private function getForm($id = "")
    {
        ...
        if ($id) {
            $query = $this->divisi_model->getById($id);
            if ($query->num_rows()>0) {
                $row = $query->row_array();
            } else {
        ...
        $this->parser->parse($template . 'divisi_form', $data);
        $this->global_libs->print_footer();
    }
	...
	public function add()
    {
        if ($_SERVER['REQUEST_METHOD'] == 'POST' && $this->validateForm()) {
            ...
            $post['iddivisi'] = $iddivisi;

            $result = $this->divisi_model->insert($post);

            // trigger add group
            $arrGroup = array('name' => $post['nama'],
                    'is_admin' => 1,
                    'iddivisi' => $iddivisi);
            $group  = $this->group_model->insert($arrGroup);
    ...
    public function edit($id = "")
    {
        ...
        if ( ($_SERVER['REQUEST_METHOD'] == 'POST') && $this->validateForm($id) ) {
            $post = purify($this->input->post());
            $this->replacer($post);
            $result = $this->divisi_model->update($id, $post);

            // trigger edit group
            $arrGroup = array('name' => $post['nama']);
            $group  = $this->group_model->updatebyDivisi($id, $arrGroup);
    ...
    {
        $result = $this->divisi_model->status($id, $status);
