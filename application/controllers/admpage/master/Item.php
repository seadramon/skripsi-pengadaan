...
class Item extends CI_Controller
{
	...
	function __construct()
	{
		...
		auth_admin();
		$this->load->model(admin_folder() . '/item_model');

        $this->folder        = admin_folder();
        $this->ctrl          = 'master/item';
        $this->template      = admin_folder() . '/master/item/';
        $this->path_uri      = admin_folder() . '/master/item';
        $this->path          = base_url() . admin_folder().'/master/item';
        $this->title         = get_admin_menu_title('item');
        $this->id_admin_menu = get_admin_menu_id('item');
	...
	function main()
	{
        ...
        $no = $lmt;
        $total_records = $this->item_model->getTotal($s_name);
        $query = $this->item_model->getAll($s_name, $per_page, $lmt);
        ...
        $this->parser->parse($template.'item', $data);
    ...
    public function add()
    {
        if ($_SERVER['REQUEST_METHOD'] == 'POST' && $this->validateForm()) {
            ...
            $result = $this->item_model->insert($post);            
    ...
    public function edit($id = "")
    {
        ...
        if ( ($_SERVER['REQUEST_METHOD'] == 'POST') && $this->validateForm($id) ) {
            ...
            $result = $this->item_model->update($id, $post);
    ...
    {
        $result = $this->item_model->status($id, $status);
    ...
    private function getForm($id = "")
    {
        ...
        if ($id) {
            $query = $this->item_model->getById($id);
            if ($query->num_rows()>0) {
                $row = $query->row_array();
            } else {
        ...
        $this->parser->parse($template . 'item_form', $data);
        $this->global_libs->print_footer();
    }
        ...
        if ($this->input->post('id')!= '') {
            if (strlen($this->input->post('id')) > 0) {
                    ...
                    if ($this->item_model->delete($id)) {
                        $this->session->set_flashdata('success_msg', 'item has been deleted.');
                    } else {
